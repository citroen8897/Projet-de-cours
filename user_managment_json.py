...
def main_menu(langue_data_base, data_base_des_users):
    ...
    if user_select_1 not in ('1', '2', '3'):
        ...
        info_de_user = {'numero': str(len(data_base_des_users) + 1),
        ...
        file_data_base = open('user_d_b.json', 'w')
        data_pour_ecrire = json.dumps(data_base_des_users, indent=4)
...
def public_info_users(langue_data_base, data_base_des_users):
    if len(data_base_des_users) > 0:
        print(f'{langue_data_base[21]} {len(data_base_des_users)} '
              f'{langue_data_base[22]}')
        
        user_select_2 = input(f'{langue_data_base[23]}')
        if user_select_2 not in (f'{langue_data_base[24]}',
            ...
            if user_select_3 not in (f'{langue_data_base[24]}',
                                     f'{langue_data_base[25]}'):
...
def total_info_user(langue_data_base, data_base_des_users):
    ...
    if user_select_5 == f'{langue_data_base[24]}':
        change_password_delete_user(langue_data_base, data_base_des_users, 
                                    user_select_4 - 1)
...
def change_password_delete_user(langue_data_base, data_base_des_users, 
                                current_user):
    user_select_6 = input(f'{langue_data_base[36]}')
    if user_select_6 not in ('4', '5', '6'):
        return change_password_delete_user(langue_data_base, 
        ...
        info_de_user = {'numero':
                        ...
                        'telephone':
                        data_base_des_users[current_user].get('telephone'),
                        'e-mail':
                        data_base_des_users[current_user].get('e-mail'),
                        'password': new_password}
        data_base_des_users[current_user] = info_de_user
        return main_menu(langue_data_base, data_base_des_users)
    
    elif user_select_6 == '5':
        user_select_7 = input(f'{langue_data_base[37]}'
        ...
        if user_select_7 != f'{langue_data_base[24]}':
            ...
            for q_n in range(len(data_base_des_users)):
                info_de_user = {'numero': q_n + 1,
                                'telephone': data_base_des_users[q_n].get(
                                    'telephone'),
                                'e-mail': data_base_des_users[q_n].get(
                                    'e-mail'),
                                'password': data_base_des_users[q_n].get(
                                    'password')}
